...
func (uq *UserQuery) QueryToken() *TokenQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, true, user.TokenTable, user.TokenColumn),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withToken; query != nil {
		...
		for _, n := range neighbors {
			fk := n.token_user
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "token_user" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "token_user" returned %v for node %v`, *fk, n.ID)
			}
