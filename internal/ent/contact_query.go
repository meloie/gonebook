...
func (cq *ContactQuery) sqlAll(ctx context.Context) ([]*Contact, error) {
	...
	if query := cq.withOwner; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].contact_owner; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "contact_owner" returned %v`, n.ID)
			}
			for i := range nodes {
...
func (cq *ContactQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if ps := cq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector, contact.ValidColumn)
			}
		}
	}
	return _spec
...
func (cq *ContactQuery) sqlQuery() *sql.Selector {
	...
	for _, p := range cq.order {
		p(selector, contact.ValidColumn)
	}
	if offset := cq.offset; offset != nil {
...
func (cgb *ContactGroupBy) sqlQuery() *sql.Selector {
	...
	for _, fn := range cgb.fns {
		columns = append(columns, fn(selector, contact.ValidColumn))
	}
